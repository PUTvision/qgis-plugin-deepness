...
MODEL_FILE_PATH = os.path.join(EXAMPLE_DATA_DIR, 'yolo11m-obb.onnx')

RASTER_FILE_PATH = os.path.join(HOME_DIR, 'examples', 'yolov7_planes_detection_google_earth', 'google_earth_planes_lawica.png')

INPUT_CHANNELS_MAPPING = create_default_input_channels_mapping_for_rgb_bands()


def test_map_processor_obb_yolo_ultralytics():
    qgs = init_qgis()
    ...
    params = DetectionParameters(
        ...
        processing_overlap=ProcessingOverlap(ProcessingOverlapOptions.OVERLAP_IN_PERCENT, percentage=15),
        model=model_wrapper,
        confidence=0.5,
        iou_threshold=0.4,
        detector_type=DetectorType.YOLO_ULTRALYTICS_OBB,
    )
...
if __name__ == '__main__':
    test_map_processor_obb_yolo_ultralytics()
    print('Done')
